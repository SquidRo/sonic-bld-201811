...
Ex:
    ./build_img2 1 ./sonic-xx.bin sonic-test.bin ./old_test
EOF
...
if [ $ROUND == 2 ]; then
    ...
    #TODO: need to modify etc/resolv.conf in $FILESYSTEM_ROOT
    #
    cat > $FILESYSTEM_ROOT/etc/resolv.conf <<- "EOF"
...
if [ $ROUND == 3 ]; then
    ...
    OLD_VER=`sed -n -e 's/\(^image_version=\)\"\(.*\)\"$/\2/p' installer/install.sh`
    ...
    [ -n "$NEW_IMG_NAME" ] || {
        NEW_IMG_NAME="sonic-test-$OLD_VER.bin"
    } 

    ./onie-mk-demo2.sh $NEW_IMG_NAME $OLD_TMPROOT installer

    [ $? -ne 0 ] || {
        echo -e "\n$NEW_IMG_NAME is generated, use it at your own risk.\n"
    }
fi ## ROUND == 3
